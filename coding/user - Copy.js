...
const UserModelInit = (sequelize) => {
    User.init({
        ...
        email: {
            ...
            validate: {
                isEmail: true,  // Added email format validation
            },
            set(email) {
                if (utils.isChallengeEnabled(datacache_1.challenges.persistedXssUserChallenge)) {
        ...
        password: {
            ...
            set(clearTextPassword) {
                // Assuming `security.hash` uses bcrypt/argon2 with salt.
                this.setDataValue('password', security.hash(clearTextPassword)); // Ensure password is securely hashed
            }
        },
        role: {
            type: sequelize_1.DataTypes.STRING,
    ...
    // Adding hooks to handle validation and challenges
