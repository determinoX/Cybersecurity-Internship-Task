    ...
    try {
        ...
        files.forEach((file) => {
            if (file.startsWith(`${key}_`)) {
                const fixFilePath = path.join(FixesDir, file);
        ...
        CodeFixes[key] = { fixes, correct };
        return CodeFixes[key];
    } catch (error) {
        console.error('Error reading fixes:', error);
        throw new Error('Error reading fixes');
    }
};
...
const checkCorrectFix = () => async (req, res, next) => {
    ...
    if (!key || !selectedFix) {
        return res.status(400).json({
            error: 'Key and selected fix are required.'
        });
    }
